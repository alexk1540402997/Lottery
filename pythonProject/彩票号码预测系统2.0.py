...
class LotteryAnalyzerComplete:
        ...
        try:
            ...
            if self.lottery_type == "ssq":
                ...
                for _ in range(num_simulations):
                    ...
                    reds = list(set(reds))  # 去重
                    if len(reds) < 6:
                        # 如果去重后不足6个，补充随机号码
                        all_nums = list(range(1, 34))
                        for num in reds:
                            all_nums.remove(num)
                        reds.extend(random.sample(all_nums, 6 - len(reds)))

                    # 根据频率权重抽样蓝球
                    blue = random.choices(
                        list(blue_freq.keys()),
                        weights=list(blue_freq.values()),
                        k=1
                    )[0]
                ...
                # 如果没有频率数据，使用均匀分布
                if not front_freq:
                    front_freq = {i: 1 for i in range(1, 36)}
                if not back_freq:
                    back_freq = {i: 1 for i in range(1, 13)}
                ...
                for _ in range(num_simulations):
                    ...
                    if len(fronts) < 5:
                        ...
                        for num in fronts:
                            all_nums.remove(num)
                        fronts.extend(random.sample(all_nums, 5 - len(fronts)))

                    # 后区抽样
                    backs = random.choices(
                        list(back_freq.keys()),
                    ...
                    if len(backs) < 2:
                        ...
                        for num in backs:
                            all_nums.remove(num)
                        backs.extend(random.sample(all_nums, 2 - len(backs)))
    ...
    def _clustering_analysis(self, recent_data: pd.DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.lottery_type == "ssq":
                # 双色球聚类分析
                # 1. 提取特征：和值、跨度、奇偶比、大小比
                red_cols = [f'red_{i}' for i in range(1, 7)]
                recent_data['sum_value'] = recent_data[red_cols].sum(axis=1)
                recent_data['span'] = recent_data[red_cols].max(axis=1) - recent_data[red_cols].min(axis=1)
                recent_data['odd_count'] = recent_data[red_cols].applymap(lambda x: x % 2 if pd.notna(x) else 0).sum(
                    axis=1)
                recent_data['small_count'] = recent_data[red_cols].applymap(
                    lambda x: 1 if pd.notna(x) and x <= 16 else 0).sum(axis=1)

                # 2. 使用KMeans进行聚类
                from sklearn.cluster import KMeans
                features = recent_data[['sum_value', 'span', 'odd_count', 'small_count']].values
                ...
                for i in range(best_k):
                    cluster_data = recent_data[clusters == i]
                    if len(cluster_data) > 0:
                        avg_sum = cluster_data['sum_value'].mean()
                        avg_span = cluster_data['span'].mean()
                        avg_odd = cluster_data['odd_count'].mean()
                        avg_small = cluster_data['small_count'].mean()
                ...
                # 5. 基于最大聚类的特征生成预测
                target_sum = int(largest_cluster['avg_sum'])
                target_odd = int(round(largest_cluster['avg_odd']))
                target_small = int(round(largest_cluster['avg_small']))

                # 生成满足条件的号码
                def generate_numbers():
                    numbers = random.sample(range(1, 34), 6)
                    if (sum(numbers) >= target_sum - 10 and sum(numbers) <= target_sum + 10 and
                            sum(1 for n in numbers if n % 2 == 1) in [target_odd - 1, target_odd, target_odd + 1] and
                            sum(1 for n in numbers if n <= 16) in [target_small - 1, target_small, target_small + 1]):
                        return numbers
                    return None
                ...
                return {
                    "method": "聚类分析",
                    "description": f"基于{best_k}个聚类的特征分析预测",
                    "statistics": {
                        "clusters": best_k,
                        "largest_cluster_size": largest_cluster['count'],
                        "total_records": len(recent_data)
                    },
                    "predictions": predictions
                }
                ...
                from sklearn.cluster import KMeans
                features = recent_data[['sum_value']].values
                ...
                # 找出最大的聚类
                cluster_sizes = [sum(clusters == i) for i in range(best_k)]
                largest_cluster_idx = cluster_sizes.index(max(cluster_sizes))
                largest_cluster_data = recent_data[clusters == largest_cluster_idx]

                target_sum = int(largest_cluster_data['sum_value'].mean())

                # 生成满足和值条件的号码
                def generate_fronts():
                    numbers = random.sample(range(1, 36), 5)
                    if target_sum - 10 <= sum(numbers) <= target_sum + 10:
                        return numbers
                    return None

                for _ in range(100):
                    fronts = generate_fronts()
                    if fronts:
                        predicted_fronts = sorted(fronts)
                        break
                else:
                    predicted_fronts = sorted(random.sample(range(1, 36), 5))
                ...
                return {
                    ...
                    "statistics": {
                        "clusters": best_k,
                        "largest_cluster_size": len(largest_cluster_data),
        ...
        try:
            ...
            if self.lottery_type == "ssq":
                ...
                for idx in range(len(recent_data) - ngram_size):
                    ...
                    for i in range(1, 7):
                        current_numbers.append(int(recent_data.iloc[idx][f'red_{i}']))
                        next_numbers.append(int(recent_data.iloc[idx + 1][f'red_{i}']))

                    # 创建键值对
                    current_key = tuple(sorted(current_numbers))
                    next_key = tuple(sorted(next_numbers))
                ...
                if not ngram_dict:
                    ...
                    return {
                        "method": "N-gram分析",
                        "description": "基于2-gram序列模式的预测（数据不足，使用随机）",
                        "statistics": {
                ...
                last_numbers = []
                for i in range(1, 7):
                    last_numbers.append(int(recent_data.iloc[-1][f'red_{i}']))
                last_key = tuple(sorted(last_numbers))

                # 4. 查找可能的下一期
                if last_key in ngram_dict and ngram_dict[last_key]:
                    # 统计后续模式
                    pattern_counter = {}
                    for pattern in ngram_dict[last_key]:
                        pattern_counter[pattern] = pattern_counter.get(pattern, 0) + 1

                    # 选择最常见的模式
                    if pattern_counter:
                        most_common = max(pattern_counter.items(), key=lambda x: x[1])[0]
                    ...
                    if red_freq:
                        hot_reds = sorted(red_freq.items(), key=lambda x: x[1], reverse=True)[:12]
                        predicted_reds = [num for num, _ in hot_reds[:6]]
                    else:
                        predicted_reds = sorted(random.sample(range(1, 34), 6))
                ...
                for idx in range(len(recent_data) - ngram_size):
                    ...
                    for i in range(1, 6):
                        current_fronts.append(int(recent_data.iloc[idx][f'front_{i}']))
                        next_fronts.append(int(recent_data.iloc[idx + 1][f'front_{i}']))

                    for i in range(1, 3):
                        current_backs.append(int(recent_data.iloc[idx][f'back_{i}']))
                        next_backs.append(int(recent_data.iloc[idx + 1][f'back_{i}']))

                    current_key = (tuple(sorted(current_fronts)), tuple(sorted(current_backs)))
                    next_key = (tuple(sorted(next_fronts)), tuple(sorted(next_backs)))
                ...
                last_backs = []
                for i in range(1, 6):
                    last_fronts.append(int(recent_data.iloc[-1][f'front_{i}']))
                for i in range(1, 3):
                    last_backs.append(int(recent_data.iloc[-1][f'back_{i}']))

                last_key = (tuple(sorted(last_fronts)), tuple(sorted(last_backs)))
                ...
                if last_key in ngram_dict and ngram_dict[last_key]:
                    ...
                    if front_freq:
                        hot_fronts = sorted(front_freq.items(), key=lambda x: x[1], reverse=True)[:10]
                        predicted_fronts = [num for num, _ in hot_fronts[:5]]
                    else:
                        predicted_fronts = sorted(random.sample(range(1, 36), 5))

                    if back_freq:
                        hot_backs = sorted(back_freq.items(), key=lambda x: x[1], reverse=True)[:4]
                        predicted_backs = [num for num, _ in hot_backs[:2]]
                    else:
                        predicted_backs = sorted(random.sample(range(1, 13), 2))
    ...
    def evaluate_prediction(self, predictions: Dict[str, Any], actual_numbers: Dict[str, List[int]]) -> Dict[str, Any]:
        """
        评估预测结果与实际开奖号码的匹配程度

        参数:
            predictions: 预测结果字典，格式同分析结果的predictions
            actual_numbers: 实际开奖号码字典
        ...
        try:
            if not predictions or not actual_numbers:
                return {"error": "预测结果或实际开奖号码为空"}

            if self.lottery_type == "ssq":
                return self._evaluate_ssq_prediction(predictions, actual_numbers)
            else:
                return self._evaluate_dlt_prediction(predictions, actual_numbers)

        except Exception as e:
            return {"error": f"评估失败: {str(e)}"}

    def _evaluate_ssq_prediction(self, predictions: Dict[str, Any],
                                 actual_numbers: Dict[str, List[int]]) -> Dict[str, Any]:
        """评估双色球预测结果"""
        result = {
            "lottery_type": "双色球",
        ...
        # 提取实际开奖号码
        actual_reds = actual_numbers.get("red", [])
        actual_blue = actual_numbers.get("blue", [0])[0] if actual_numbers.get("blue") else 0

        if not actual_reds or actual_blue == 0:
            result["error"] = "实际开奖号码无效"
            return result
        ...
        if isinstance(predictions, dict):
            ...
            for key, value in predictions.items():
                if isinstance(value, dict):
                    if 'red' in value and 'blue' in value:
                        predicted_reds = value['red'] if isinstance(value['red'], list) else []
                        predicted_blues = value['blue'] if isinstance(value['blue'], list) else []
                        break
                elif 'red' in predictions and 'blue' in predictions:
                    predicted_reds = predictions['red'] if isinstance(predictions['red'], list) else []
                    predicted_blues = predictions['blue'] if isinstance(predictions['blue'], list) else []
                    break

        # 确保预测号码格式正确
        if not isinstance(predicted_reds, list):
            predicted_reds = []
        if not isinstance(predicted_blues, list):
            predicted_blues = []

        # 提取主要预测结果（取前6个红球和前1个蓝球）
        main_predicted_reds = predicted_reds[:6] if len(predicted_reds) >= 6 else predicted_reds
        main_predicted_blues = predicted_blues[:1] if predicted_blues else [0]

        # 1. 红球命中评估
        red_hit_count = 0
        red_hit_numbers = []
        for num in main_predicted_reds:
            if num in actual_reds:
                red_hit_count += 1
                red_hit_numbers.append(num)

        red_hit_rate = red_hit_count / 6 if len(main_predicted_reds) >= 6 else 0

        # 2. 蓝球命中评估
        blue_hit = 0
        blue_hit_number = 0
        if main_predicted_blues and main_predicted_blues[0] > 0:
            if main_predicted_blues[0] == actual_blue:
                blue_hit = 1
                blue_hit_number = main_predicted_blues[0]

        # 3. 计算和值偏差
        predicted_sum = sum(main_predicted_reds) if main_predicted_reds else 0
        actual_sum = sum(actual_reds)
        sum_deviation = abs(predicted_sum - actual_sum) if predicted_sum > 0 else 0

        # 4. 计算奇偶比
        predicted_odd_count = sum(1 for n in main_predicted_reds if n % 2 == 1) if main_predicted_reds else 0
        actual_odd_count = sum(1 for n in actual_reds if n % 2 == 1)
        odd_ratio_deviation = abs(predicted_odd_count - actual_odd_count)

        # 5. 计算大小比（1-16为小，17-33为大）
        predicted_small_count = sum(1 for n in main_predicted_reds if n <= 16) if main_predicted_reds else 0
        actual_small_count = sum(1 for n in actual_reds if n <= 16)
        small_ratio_deviation = abs(predicted_small_count - actual_small_count)
        ...
        def calculate_ac_value(numbers):
            ...
            for i in range(n):
                for j in range(i + 1, n):
                    diffs.append(abs(numbers[j] - numbers[i]))

            unique_diffs = set(diffs)
            return len(unique_diffs) - (n - 1)

        predicted_ac = calculate_ac_value(sorted(main_predicted_reds)) if len(main_predicted_reds) >= 2 else 0
        actual_ac = calculate_ac_value(sorted(actual_reds))
        ac_deviation = abs(predicted_ac - actual_ac) if predicted_ac > 0 else 0

        # 7. 计算总分
        # 红球命中权重：6分，蓝球命中权重：1分，其他指标权重：3分
        total_score = (
                red_hit_count * 1.0 +  # 每个红球命中得1分
                blue_hit * 1.0 +  # 蓝球命中得1分
                max(0, 1 - sum_deviation / 100) * 1.0 +  # 和值偏差评分
                max(0, 1 - odd_ratio_deviation / 6) * 0.5 +  # 奇偶比偏差评分
                max(0, 1 - small_ratio_deviation / 6) * 0.5  # 大小比偏差评分
        )
        ...
        result.update({
            "basic_info": {
                ...
                "predicted_reds": sorted(main_predicted_reds),
                "predicted_blues": main_predicted_blues
            },
            "hit_analysis": {
                "red_hit_count": red_hit_count,
                "red_hit_numbers": sorted(red_hit_numbers),
                "red_hit_rate": round(red_hit_rate, 4),
                "blue_hit": blue_hit,
                "blue_hit_number": blue_hit_number,
                "total_hit_score": red_hit_count + blue_hit
            },
            "feature_analysis": {
                "sum_deviation": sum_deviation,
                "predicted_sum": predicted_sum,
                "actual_sum": actual_sum,
                "odd_ratio_deviation": odd_ratio_deviation,
                "predicted_odd_count": predicted_odd_count,
                "actual_odd_count": actual_odd_count,
                "small_ratio_deviation": small_ratio_deviation,
                "predicted_small_count": predicted_small_count,
                "actual_small_count": actual_small_count,
                "ac_deviation": ac_deviation,
                "predicted_ac": predicted_ac,
                "actual_ac": actual_ac
            },
            "score_summary": {
                "total_score": round(total_score, 4),
                "max_possible_score": 9.0,  # 6红球 + 1蓝球 + 1和值 + 0.5奇偶 + 0.5大小
                "score_rate": round(total_score / 9.0, 4)
            },
            "additional_info": {
                "has_prediction": len(main_predicted_reds) > 0,
                "predicted_red_count": len(main_predicted_reds),
                "predicted_blue_count": len(main_predicted_blues)
            }
        })

        return result

    def _evaluate_dlt_prediction(self, predictions: Dict[str, Any],
                                 actual_numbers: Dict[str, List[int]]) -> Dict[str, Any]:
        """评估大乐透预测结果"""
        result = {
            "lottery_type": "大乐透",
        ...
        # 提取实际开奖号码
        actual_fronts = actual_numbers.get("front", [])
        actual_backs = actual_numbers.get("back", [])

        if not actual_fronts or not actual_backs:
            result["error"] = "实际开奖号码无效"
            return result
        ...
        if isinstance(predictions, dict):
            for key, value in predictions.items():
                if isinstance(value, dict):
                    if 'front' in value and 'back' in value:
                        predicted_fronts = value['front'] if isinstance(value['front'], list) else []
                        predicted_backs = value['back'] if isinstance(value['back'], list) else []
                        break
                elif 'front' in predictions and 'back' in predictions:
                    predicted_fronts = predictions['front'] if isinstance(predictions['front'], list) else []
                    predicted_backs = predictions['back'] if isinstance(predictions['back'], list) else []
                    break

        # 确保预测号码格式正确
        if not isinstance(predicted_fronts, list):
            predicted_fronts = []
        if not isinstance(predicted_backs, list):
            predicted_backs = []

        # 提取主要预测结果
        main_predicted_fronts = predicted_fronts[:5] if len(predicted_fronts) >= 5 else predicted_fronts
        main_predicted_backs = predicted_backs[:2] if len(predicted_backs) >= 2 else predicted_backs

        # 1. 前区命中评估
        front_hit_count = 0
        front_hit_numbers = []
        for num in main_predicted_fronts:
            if num in actual_fronts:
                front_hit_count += 1
                front_hit_numbers.append(num)

        front_hit_rate = front_hit_count / 5 if len(main_predicted_fronts) >= 5 else 0

        # 2. 后区命中评估
        back_hit_count = 0
        back_hit_numbers = []
        for num in main_predicted_backs:
            if num in actual_backs:
                back_hit_count += 1
                back_hit_numbers.append(num)

        back_hit_rate = back_hit_count / 2 if len(main_predicted_backs) >= 2 else 0

        # 3. 计算和值偏差
        predicted_sum = sum(main_predicted_fronts) if main_predicted_fronts else 0
        actual_sum = sum(actual_fronts)
        sum_deviation = abs(predicted_sum - actual_sum) if predicted_sum > 0 else 0

        # 4. 计算总分
        total_score = (
                front_hit_count * 1.0 +  # 每个前区命中得1分
                back_hit_count * 2.0 +  # 每个后区命中得2分（权重更高）
                max(0, 1 - sum_deviation / 200) * 1.0  # 和值偏差评分
        )

        result.update({
            "basic_info": {
                "actual_fronts": sorted(actual_fronts),
                "actual_backs": sorted(actual_backs),
                "predicted_fronts": sorted(main_predicted_fronts),
                "predicted_backs": sorted(main_predicted_backs)
            },
            "hit_analysis": {
                "front_hit_count": front_hit_count,
                "front_hit_numbers": sorted(front_hit_numbers),
                "front_hit_rate": round(front_hit_rate, 4),
                "back_hit_count": back_hit_count,
                "back_hit_numbers": sorted(back_hit_numbers),
                "back_hit_rate": round(back_hit_rate, 4),
                "total_hit_score": front_hit_count + back_hit_count * 2
            },
            "feature_analysis": {
                "sum_deviation": sum_deviation,
                "predicted_sum": predicted_sum,
            ...
            "score_summary": {
                "total_score": round(total_score, 4),
                "max_possible_score": 9.0,  # 5前区 + 4后区（2 * 2） + 1和值
                "score_rate": round(total_score / 9.0, 4)
            },
            "additional_info": {
                "has_prediction": len(main_predicted_fronts) > 0,
                "predicted_front_count": len(main_predicted_fronts),
                "predicted_back_count": len(main_predicted_backs)
            }
        })
    ...
    def _save_method_details(self, writer) -> None:
        ...
        method_details = {
            ...
            "method_6": ("蒙特卡罗模拟", self.analysis_results.get('method_6', {})),  # 新增
            "method_7": ("聚类分析", self.analysis_results.get('method_7', {})),  # 新增
            "method_8": ("N-gram分析", self.analysis_results.get('method_8', {})),  # 新增
    ...
    def _save_prediction_summary(self, writer) -> None:
        ...
        # 各方法结果
        method_order = ["method_1", "method_2", "method_3", "method_4", "method_5", "method_6", "method_7", "method_8",
                        "comprehensive"]
        method_names = {
            "method_1": "统计概率分析",
            "method_2": "时间序列分析",
    ...
    def analyze_data(self):
        ...
        try:
            ...
            for method_name, result in results.items():
                ...
                elif method_name == "method_6":
                    self._display_result("蒙特卡罗", result)
                elif method_name == "method_7":
                    self._display_result("聚类分析", result)
                elif method_name == "method_8":
                    self._display_result("N-gram", result)
